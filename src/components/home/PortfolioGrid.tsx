...
const images = [
  { src: '/public/wraps.png', title: 'Wraps', tag: 'Wraps' },
  { src: '/public/decals.png', title: 'Decals', tag: 'Decals' },
  { src: '/public/websites.png', title: 'Websites', tag: 'Websites' },
  { src: '/public/my-branded-truck.png', title: 'Vehicles', tag: 'Wraps' },
];
  ...
  return (
    <Box>
        ...
        {list.map((img, i) => (
          <Grid item xs={6} sm={4} md={3} key={i}>
            <motion.div initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }}>
              <ImageListItem sx={{ borderRadius: 2, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
                <img src={img.src} alt={img.title} loading="lazy" style={{ width: '100%', height: 'auto', display: 'block' }} />
      ...
      <Box sx={{ textAlign: 'center', mt: 2 }}>
        <Button variant="contained" onClick={onCTA}>Get your free mockup</Button>
      </Box>
    </Box>
  );
};
